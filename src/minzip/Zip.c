                                 ...
                                 void *cookie)
{
    ...
    while (true) {
		ssize_t n;
        TEMP_FAILURE_RETRY_NONGNU(n, write(fd, data+soFar, dataLen-soFar));
        if (n <= 0) {
...
#define UNZIP_FILEMODE 0644
        ...
        if (pEntry->fileName[pEntry->fileNameLen-1] != '/') {
            ...
            int ret = dirCreateHierarchy(
                    targetFile, UNZIP_DIRMODE, (struct utimbuf*) timestamp, true);
            if (ret != 0) {
            ...
            int fd;
			#if defined(_WIN32)
			fd = open(targetFile, O_CREAT|O_WRONLY|O_TRUNC, UNZIP_FILEMODE);
			#else
			fd = open(targetFile, O_CREAT|O_WRONLY|O_TRUNC|O_SYNC, UNZIP_FILEMODE);
			#endif
            ...
            if (timestamp != NULL && utime(targetFile, (struct utimbuf*) timestamp)) {
