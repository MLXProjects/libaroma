	...
	if (n>=8){
		...
		for (i=0;i<n-left;i+=8){
		 ...
		 __m128i o = _mm_or_si128(_mm_or_si128(rbl,gbl),bbl);
			...
			_mm_storeu_si128((__m128i*) (dst+i), o);
		}
	}
	/* leftover */
	if (left>0){
...
static inline void libaroma_alpha_const_line(
	...
	if (n>=8){
		...
		for (i=0;i<n-left;i+=8){
			...
			/* out value */
			_mm_storeu_si128((__m128i*) (dst+i), _mm_or_si128(_mm_or_si128(rbl,gbl),bbl));
		}
	}
	/* leftover */
	if (left>0){
...
static inline void libaroma_alpha_black(
	...
	if (n>=8){
		...
		for (i=0;i<n-left;i+=8){
			...
			/* out value */
			_mm_storeu_si128((__m128i*) (dst+i), _mm_or_si128(_mm_or_si128(rtl,gtl),btl));
		}
	}
	/* leftover */
	if (left>0){
