 ...
 */
FT_Library _libaroma_font_instance;
 ...
 */
_LIBAROMA_FONT_FACE _libaroma_font_faces[_LIBAROMA_FONT_MAX_FACE];

/* MUTEXES */
LIBAROMA_MUTEX __libaroma_text_locks[3];

void __libaroma_text_locker(byte font, byte lock){
	if (lock){
...
void __libaroma_text_locker_init(byte destroy){
	int i;
	for (i=0;i<3;i++){
		if (!destroy){
